...
const actions = {
  ...
  addActivitesFilieresSecteurs({ commit }, FormData) {
    // request
    console.log("ajouter une FAS");
    let data = new FormData();
    data.append("activite", this.activite);
    data.append("filiere", this.filiere);
    data.append("secteur", this.secteur);
    axios
      .post("http://localhost:8000/rsu/StdTables/addASF", data)
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          axios.get("http://localhost:8000/rsu/StdTables/listeActivitesFilieresSecteurs").then(res => {
